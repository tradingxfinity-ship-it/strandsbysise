...
def product_card(p, order):
    ...
    lace = str(p.get("lace", "")).strip()
    meta = " · ".join(x for x in (lace, p.get("length", ""), weight, closure) if x)
    closure_spec = '\n              <li class="spec">%s closure</li>' % attr(closure) if closure else ""
    lace_spec = '\n              <li class="spec">%s lace</li>' % attr(lace) if lace else ""
    url = product_url(p)
    ...
    return """        <article class="product-card" data-reveal data-category="{cat}" data-price="{price}" data-rating="{rating}" data-order="{order}">
          ...
          <div class="product-card__body">
            ...
            <ul class="spec-list">
              ...
              <li class="spec">{weight}</li>{lace_spec}{closure_spec}
            </ul>
        ...
        weight=attr(weight), closure_spec=closure_spec, lace_spec=lace_spec,
...
def fixed_spec_row(label, value):
    """A read-only 'label ....... value' row on the product page, rendered
    only when Sise set a value — so unset specs simply don't appear."""
...
def closure_row(p):
    return fixed_spec_row("Closure", p.get("closure", ""))


def lace_row(p):
    return fixed_spec_row("Lace type", p.get("lace", ""))
...
def render_product(p):
    ...
    tokens = {
        ...
        "PDP_LACE": attr(p.get("lace", "")),
        "PDP_LACE_ROW": lace_row(p),
        "PDP_META": attr(" · ".join(
            x for x in ["Natural Black", p.get("lace", ""), p.get("length", ""),
                        weight_display(p), p.get("closure", "")] if x)),
    }
